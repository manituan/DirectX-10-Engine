...
bool TerrainShaderClass::Initialize(ID3D10Device* device, HWND hwnd)
{
	...
	// Initialize the shader that will be used to draw the polygons.
	result = InitializeShader(device, hwnd, L"../Manituan/terrain.fx");
	if(!result)
	{
		return false;
	}
...
void TerrainShaderClass::Render(ID3D10Device* device, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, 
								D3DXMATRIX projectionMatrix, ID3D10ShaderResourceView* texture, D3DXVECTOR4 ambientColor, 
								D3DXVECTOR4 diffuseColor, D3DXVECTOR3 lightDirection)
{
	// Set the shader parameters that it will use for rendering.
	SetShaderParameters(worldMatrix, viewMatrix, projectionMatrix, texture, ambientColor, diffuseColor, lightDirection);
...
bool TerrainShaderClass::InitializeShader(ID3D10Device* device, HWND hwnd, WCHAR* filename)
{
	...
	// Get pointers to the color variables and light direction inside the shader.
...
void TerrainShaderClass::SetShaderParameters(D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix, 
											 ID3D10ShaderResourceView* texture, D3DXVECTOR4 ambientColor, 
											 D3DXVECTOR4 diffuseColor, D3DXVECTOR3 lightDirection)
{
...
void TerrainShaderClass::RenderShader(ID3D10Device* device, int indexCount)
{
    ...
    // Go through each pass in the technique (should be just one currently) and render the polygons.
