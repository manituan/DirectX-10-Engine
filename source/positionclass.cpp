...
void PositionClass::MoveLeft(bool keydown)
{
	...
	if(keydown)
	{
		m_lateralSpeed += m_frameTime * 0.02f;

		if(m_lateralSpeed > (m_frameTime * 0.03f))
		{
			m_lateralSpeed = m_frameTime * 0.03f;
		}
	}
	else
	{
		m_lateralSpeed -= m_frameTime * 0.0007f;
	...
	// Update the position.
	m_positionX += m_lateralSpeed;
	m_positionZ -= m_lateralSpeed;
